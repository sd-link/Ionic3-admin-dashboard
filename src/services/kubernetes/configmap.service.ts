...
export class ConfigMapService {
    ...
    private getUrl(key, context: any = ''): string {
        return this._endpoint.getUrl(key, 'configmap', context);
    }
    ...
    listConfigMaps(projectId, queryString = null) {
        ...
        const request = this.getAuthToken()
            ...
            .map(res => res.json())
            .catch((error: any) => {
                return Observable.throw(error.json() || 'Error retrieving list of configmaps');
            });
        return this.cache.loadFromObservable(url, request, "projectList");
    }

    createConfigMap(body, projectId) {
        ...
        return this.getAuthToken()
            ...
            .catch((error: any) => {
                return Observable.throw(error.json() || 'Error creating configmap')
            });
    }
}
