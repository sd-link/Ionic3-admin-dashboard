...
export class SecretService {
    ...
    private getUrl(key, context: any = ''): string {
        return this._endpoint.getUrl(key, 'secret', context);
    }
    ...
    listSecrets(projectId, queryString = null) {
        ...
        const request = this.getAuthToken()
            ...
            .map(res => res.json())
            .catch((error: any) => {
                return Observable.throw(error.json() || 'Error listing secret');
            });
    ...
    createSecret(body, projectId) {
        ...
        return this.getAuthToken()
            ...
            .catch((error: any) => {
                return Observable.throw(error.json() || 'Error creating secret')
            });
    }
}
