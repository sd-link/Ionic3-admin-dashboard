...
export class ServiceService {
    ...
    private getUrl(key, context: any = ''): string {
        return this._endpoint.getUrl(key, 'service', context);
    }
    ...
    listServices(projectId, queryString = null) {
        ...
        const request = this.getAuthToken()
            ...
            .map(res => res.json())
            .catch((error: any) => {
                return Observable.throw(error.json() || 'Error retrieving list of services');
            });
